...
struct OpenGLUrdfVisualizer {
  ...
  OpenGLUrdfVisualizer(int width = 1024, int height = 768)
      : m_uid(1234), m_opengl_app("test", width, height) {
  ...
  void load_obj(const std::string& obj_filename, const TinyVector3f& pos,
                const TinyQuaternionf& orn, const TinyVector3f& scaling,
                std::vector<int>& instance_ids) {
    ...
    for (int i = 0; i < shapes.size(); i++) {
      ...
      if (shapes[i].mesh.material_ids.size()) {
        const tinyobj::material_t& mat =
            materials[shapes[i].mesh.material_ids[0]];
    ...
    for (int vis_index = 0; vis_index < (int)link.urdf_visual_shapes.size();
      ...
      if (v.geometry.geom_type == TINY_MESH_TYPE) {
        ...
        std::string org_obj_filename =
            m_path_prefix + v.geometry.m_mesh.m_file_name;
    ...
    for (int v = 0; v < body->m_visual_uids2.size(); v++) {
      ...
      if (m_b2vis.find(visual_id) != m_b2vis.end()) {
        ...
        for (int i = 0; i < viz.instance_ids.size(); i++) {
          ...
          TinyVector3f pos(geom_X_world.m_translation[0],
                           geom_X_world.m_translation[1],
                           geom_X_world.m_translation[2]);
