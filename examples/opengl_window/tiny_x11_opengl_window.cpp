...
TinyX11OpenGLWindow::TinyX11OpenGLWindow()
...
void TinyX11OpenGLWindow::enable_opengl() {
  if (forceOpenGL3) {
...
*/
      ...
      if (!ctxErrorOccurred && ctx)
        ...
        // less than version 3.0. GLX_CONTEXT_MAJOR_VERSION_ARB = 1
        context_attribs[1] = 1;
...
void TinyX11OpenGLWindow::disable_opengl() {
  glXMakeCurrent(m_data->m_dpy, None, NULL);
  glXDestroyContext(m_data->m_dpy, m_data->m_glc);
}

void TinyX11OpenGLWindow::create_window(const TinyWindowConstructionInfo& ci) {
...
void TinyX11OpenGLWindow::close_window() {
  disable_opengl();
...
int TinyX11OpenGLWindow::get_ascii_code_from_virtual_keycode(int keycode) {
  int result = 0;
  ...
  switch (key) {
    ...
    case XK_Control_L:
    case XK_Control_R: {
      return TINY_KEY_CONTROL;
    }
    case XK_Left:
      return TINY_KEY_LEFT_ARROW;
    case XK_Right:
      return TINY_KEY_RIGHT_ARROW;
    case XK_Up:
      return TINY_KEY_UP_ARROW;
    case XK_Down:
      return TINY_KEY_DOWN_ARROW;
...
bool TinyX11OpenGLWindow::is_modifier_key_pressed(int key) {
...
void TinyX11OpenGLWindow::pump_message() {
  int buttonState = 1;
  ...
  while (MyXPending(m_data->m_dpy)) {
    ...
    switch (m_data->m_xev.type) {
      case KeyPress: {
        int keycode =
            get_ascii_code_from_virtual_keycode(m_data->m_xev.xkey.keycode);
        switch (keycode) {
          case TINY_KEY_ALT:
...
void TinyX11OpenGLWindow::render_all_objects() {}
...
float TinyX11OpenGLWindow::get_time_in_seconds() { return 0.f; }

bool TinyX11OpenGLWindow::requested_exit() const { return m_requestedExit; }
...
void TinyX11OpenGLWindow::set_wheel_callback(TinyWheelCallback wheelCallback) {
...
void TinyX11OpenGLWindow::set_mouse_button_callback(
...
TinyMouseMoveCallback TinyX11OpenGLWindow::get_mouse_move_callback() {
  return m_data->m_mouseMoveCallback;
}
TinyMouseButtonCallback TinyX11OpenGLWindow::get_mouse_button_callback() {
  return m_data->m_mouseButtonCallback;
}
TinyResizeCallback TinyX11OpenGLWindow::get_resize_callback() {
  return m_data->m_resizeCallback;
}
TinyWheelCallback TinyX11OpenGLWindow::get_wheel_callback() {
  return m_data->m_wheelCallback;
}

TinyKeyboardCallback TinyX11OpenGLWindow::get_keyboard_callback() {
  return m_data->m_keyboardCallback;
}

int TinyX11OpenGLWindow::get_width() const {
  if (m_data) return m_data->m_glWidth;
  return 0;
}
int TinyX11OpenGLWindow::get_height() const {
...
int TinyX11OpenGLWindow::file_open_dialog(char* filename, int maxNameLength) {
  int len = 0;
  FILE* output = popen(
