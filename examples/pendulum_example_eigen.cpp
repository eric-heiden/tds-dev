...
int main(int argc, char* argv[]) {
  ...
  mb->q() = VectorX::Zero(mb->dof());
  mb->qd() = VectorX::Zero(mb->dof_qd());
  mb->tau() = VectorX::Zero(mb->dof_qd());
  mb->qdd() = VectorX::Zero(mb->dof_qd());
  ...
  while (!app.m_window->requested_exit()) 
  {
...

    {
      ...
      // M.print("M"); // No supported in Eigen
      if (mb->qd()[0] < -1e4) {
        assert(0);
      }
    }
    ...
    for (int b = 0; b < mbbodies.size(); b++) {
        ...
        Quarternion rot(geom_X_world.rotation);
