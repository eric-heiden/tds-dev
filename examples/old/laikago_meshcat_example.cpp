...
int main(int argc, char* argv[]) {
  ...
  if (useLaikago) {
    ...
    std::string texture_path = search_path + std::string("checker_blue.png");
    meshcat_viz.convert_visuals(urdf_data, texture_path);
  ...
  TinyMultiBody<double, DoubleUtils>* mb = world.create_multi_body();
  {
    ...
    std::string texture_path =
        search_path + std::string("laikago/laikago_tex.jpg");
    meshcat_viz.convert_visuals(urdf_data, texture_path);
      ...
      if (1) {
        ...
        // sync physics to visual transforms
        {
          for (int b = 0; b < mbbodies.size(); b++) {
            ...
            meshcat_viz.sync_visual_transforms(body);
          }
        }
