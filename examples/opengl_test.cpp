...
int main(int argc, char* argv[]) {
  ...
  int inst = app.m_renderer->register_graphics_instance(shape, pos, orn, color,
                                                        scaling);
  ...
  bool ret =
      tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err,
                       laikago_urdf_filename.c_str(), basepath, triangulate);
  ...
  for (int i = 0; i < shapes.size(); i++) {
    ...
    shape = app.m_renderer->register_shape(&vertices[0].x, vertices.size(),
