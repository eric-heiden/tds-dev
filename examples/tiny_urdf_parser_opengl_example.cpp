...
int main(int argc, char* argv[]) {
  ...
  if (mb.dof() >= 12) {
    for (int cc = 0; cc < 12; cc++) {
      mb.q(start_index + cc) = initial_poses[cc];
    }
  }
  ...
  while (!visualizer.m_opengl_app.m_window->requested_exit()) {
      ...
      if (do_sim) {
          ...
          if (1) {
              ...
              int num_targets = mb.dof_actuated() - qd_offset;
              std::vector<double> q_targets;
              q_targets.resize(mb.dof_actuated());
              ...
              for (const auto &link : mb) {
                  if (link.joint_type != JOINT_FIXED) {
                      ...
                      double q_actual = mb.q(q_offset);
                      double qd_actual = mb.qd(qd_offset);
