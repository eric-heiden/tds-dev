...
struct TinyDualDoubleUtils {
  ...
  static TinyDualDouble copysign(const TinyDualDouble& x,
                                 const TinyDualDouble& y) {
    return TinyDualDouble(::copysign(x.real(), y.real()));
  }

  static TinyDualDouble asin(const TinyDualDouble& z) {
    return TinyDualDouble(::asin(z.real()),
                          z.dual() / ::sqrt(double(1) - z.real() * z.real()));
  }

  static TinyDualDouble atan2(const TinyDualDouble& y,
                              const TinyDualDouble& x) {
    TinyDualDouble z = y / x;
    bool neg_x = x.real() < 0;
    bool neg_y = y.real() < 0;

    double quadrant = neg_x ? (neg_y ? -M_PI : M_PI) : 0;
    return TinyDualDouble(atan(z.real()) + quadrant,
                          z.dual() / (double(1) + (z.real() * z.real())));
  }
